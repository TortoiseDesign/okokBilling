...
local Webhook = 'https://discord.com/api/webhooks/918355543624671293/D4F8ZO46ZR3GrqOthLPr4zQYK8nbI_br1GKP2eDOrv_-gYPv2HL4QbvcpZX2JxU5RRs_'
...
AddEventHandler("okokBilling:PayInvoice", function(invoice_id)
	...
	MySQL.Async.fetchAll('SELECT * FROM okokBilling WHERE id = @id', {
		...
		if playerMoney < invoices.invoice_value then
			--TriggerClientEvent('okokNotify:Alert', _source, "BILLING", "Bạn không có đủ tiền!", 10000, 'error')
			TriggerClientEvent('QBCore:Notify', _source, "Bạn không có đủ tiền", "error")
			...
			TriggerClientEvent('QBCore:Notify', _source, "Đã thanh toán hóa đơn thành công", "success")
...
AddEventHandler("okokBilling:CancelInvoice", function(invoice_id)
	...
	MySQL.Async.fetchAll('SELECT * FROM okokBilling WHERE id = @id', {
		...
		-- TriggerClientEvent('QBCore:Notify', _source, "Đã hủy hóa đơn", "info")

		TriggerClientEvent('okokNotify:Alert', _source, "Hóa đơn", "Đã hủy hóa đơn", 10000, 'info')
		if Webhook ~= '' then
...
AddEventHandler("okokBilling:CreateInvoice", function(data)
	...
	if Config.LimitDate then
		MySQL.Async.insert('INSERT INTO okokBilling (receiver_identifier, receiver_name, author_identifier, author_name, society, society_name, item, invoice_value, status, notes, sent_date, limit_pay_date) VALUES (@receiver_identifier, @receiver_name, @author_identifier, @author_name, @society, @society_name, @item, @invoice_value, @status, @notes, CURRENT_TIMESTAMP(), DATE_ADD(CURRENT_TIMESTAMP(), INTERVAL @limit_pay_date DAY))', {
			...
			TriggerClientEvent('okokNotify:Alert', target.PlayerData.source, "Hóa đơn", "[/hoadon] Bạn nhận được yêu cầu thanh toán hóa đơn mới", 10000, 'info')
			-- TriggerClientEvent('QBCore:Notify', target.PlayerData.source, "[/hoadon] Bạn nhận được yêu cầu thanh toán hóa đơn mới", "info")
				...
				if xPlayer == nil then
					MySQL.Async.fetchAll('SELECT money FROM players WHERE citizenid = @id', {
						...
						MySQL.Async.execute('UPDATE players SET money = @playerAccount WHERE citizenid = @target', {
							...
							TriggerEvent("qb-bossmenu:server:okokBillingDeposit", v.society_name, invoice_value)
	...
	local information = {
		{
			...
			["title"] = 'Hóa đơn #'..data.id..' đã được thanh toán',
			["description"] = '**Người nhận:** '..data.player_name..'\n**Giá trị:** '..data.value..'$\n**Lý do:** '..data.item..'\n**Công ty:** '..data.society,
	...
	local information = {
		{
			...
			["title"] = 'Hóa đơn #'..data.id..' đã bị hủy',
			["description"] = '**Hủy bởi:** '..data.name..'\n\n**Người nhận:** '..data.player_name..'\n**Giá trị:** '..data.value..'$\n**Lý do:** '..data.item..'\n**Công ty:** '..data.society,
	...
	local information = {
		{
			...
			["title"] = 'Hóa đơn #'..data.id..' đã được tạo',
			["description"] = '**Tạo bởi:** '..data.name..'\n**Công ty:** '..data.society..'\n\n**Người nhận:** '..data.player_name..'\n**Giá trị:** '..data.value..'$\n**Lý do:** '..data.item,
	...
	local information = {
		{
			...
			["title"] = 'Hóa đơn #'..data.id..' đã được thanh toán tự động',
			["description"] = '**Người nhận:** '..data.player_name..'\n**Giá trị:** '..data.value..'$\n**Lý do:** '..data.item..'\n**Công ty:** '..data.society,
